...
public class ArbolBB {
    ...
    //se crea el nodo raiz y la variable de nodos
    private Nodo raiz;
    int num_nodos;
    int alt;

    public ArbolBB() {
        raiz = null;        //se inicializa la raiz como nula
    }
    
    public boolean agregar(int dato) {
        Nodo nuevo = new Nodo(dato, null, null);
        insertar(nuevo, raiz);  //se envia el nuevo nodo y la raiz para agregar
        return true;
    }
    
    //Metodo para insertar un dato en el arbol...no acepta repetidos :)
    public void insertar(Nodo nuevo, Nodo pivote) {
        if (this.raiz == null) {                        //se verifica si la raiz es nula, o sea
            raiz = nuevo;                               //si es el primer dato del árbol
        } else {
    ...
    public Nodo getRaiz() {
        return raiz;            //se obtiene la raiz
    }
